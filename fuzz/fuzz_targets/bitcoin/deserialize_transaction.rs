...
fn do_test(data: &[u8]) {
    let tx_result: Result<kaon::blockdata::transaction::Transaction, _> =
        kaon::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {}
        Ok(mut tx) => {
            let ser = kaon::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            let calculated_weight = tx.weight().to_wu() as usize;
            for input in &mut tx.input {
                input.witness = kaon::blockdata::witness::Witness::default();
            }
            let no_witness_len = kaon::consensus::encode::serialize(&tx).len();
