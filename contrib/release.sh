...
main () {
    for crate in "internals" "hashes" "kaon"; do
...
publish_dry_run() {
    ...
    if [ "$crate" == "hashes" ]; then
        cargo publish -p "kaon_hashes" --dry-run
    elif [ "$crate" == "internals" ]; then
        cargo publish -p "kaon-internals" --dry-run
    elif [ "$crate" == "kaon" ]; then
        cargo publish -p "kaon" --dry-run
    fi
}
